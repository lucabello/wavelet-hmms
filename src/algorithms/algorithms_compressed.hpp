...
*/
void evaluation_compressed(Model& m, Compressor *c){
    wahmm::real_t **logForward;
    wahmm::real_t logEvaluation;
    size_t numberOfStates = m.mStates.size();

    LOG(DEBUG) << "+++ Compressed Evaluation Problem +++" << std::endl;
...
* @param tofile save results to file
*/
void decoding_compressed(Model &m, Compressor *c){
    ...
    LOG(DEBUG) << "[>] +++ Compressed Decoding Problem +++" << std::endl;
    ...
    for(; blockCounter > 0; blockCounter--){
        ...
        if(currentState >= 0)
            ...
            LOG(ERROR) << "Impossible Viterbi path!" << std::endl;
            break;
        }
        viterbiPath.push_front(currentState);
    }
...
void training_compressed(Model& m, Compressor *c, wahmm::real_t thresh,
    ...
    LOG(DEBUG) << "+++ Compressed Training Problem +++" << std::endl;
